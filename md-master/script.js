var tableDataOrig;
    ...
    function closestDate(dates, backYear, intrstdDt) {
        if(intrstdDt==""){
            var sdf = new Date(); sdf.setHours(0, 0, 0, 0); //searchDateForward
            var sdb = new Date(); sdb.setHours(0, 0, 0, 0); //searchDateBackward
    ...
    window.onscroll = function(ev) {
        ...
        if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight) {
            if(bottomRow!=tableData.length){
                ...
                bottomRow += 500; if(bottomRow>tableData.length) bottomRow=tableData.length;
        ...
        if (window.scrollY < 10) {
            if(topRow!=0){
                ...
                topRow -= 500; if(topRow<0) topRow=0;
